function [sysAbs] = GridSpace_nonlin_tensor_v2(sys, uhat,l, tol)
% [sysAbs] = GridSpace(sys, uhat,l, tol) 
%
% Inputs: 
...
hz  = arrayfun( @(zl,zu,gridsize)  zl+gridsize/2:gridsize:zu, Zl,Zu,gridSize,'UniformOutput',false);
...
for k = 1:length(uhat) % for each control action
    z_n = Uz'*sys.f_det(XhatSpace,uhat(:,k)*ones(1,size(XhatSpace,2)));
    z_n_ind = ones(dim,1)+floor(diag(gridSize.^-1)*(z_n-Zl'));
    indices = min([[1;1]<=z_n_ind;l'>=z_n_ind],[],1);
    zi_indices= arrayfun(@(i)z_n_ind(i,indices),1:dim,'UniformOutput',false);
