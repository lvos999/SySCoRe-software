function [epsilonBounds] = ComputeEpsilonBounds_affine(sysLTI,sysLTIr,mu,sigma,uuf,Z,P)
    ...
    for k = 1:length(lambda_v)
        ...
        for j = 1:size(Zset,1)
            ...
            M3 =   [lambda*Dinv,zeros(dim,1), Dinv*A'+E'*B'+L'*Bwr'*P';
                    zeros(1,dim), (1-lambda)*eps_sq, -eps_sq*(z'+(Qc-P*Qcr)');
                    A*Dinv+B*E+P*Bwr*L, -eps_sq*(z+(Qc-P*Qcr)), Dinv];
            LMI = [LMI; (M3>=0)];
        end
